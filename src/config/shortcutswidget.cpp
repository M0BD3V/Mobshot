...
void ShortcutsWidget::checkPrintScreenForcesSnipping()
{
    if (!isPrintScreenKeyForSnippingDisabled() &&
        ...
        msgBox.setText(tr("It seems, that Windows forces to open its screenshot"
                          ...
                          "you like to disable this so that Mobshot can use "
                          "the 'Print Screen' key?") +
                       "\n\n" +
                       tr("Mobshot must be restarted for changes to take "
                          "effect."));
        ...
        if (msgBox.clickedButton() == yesBtn) {
            if (!disablePrintScreenKeyForSnipping()) {
                QMessageBox::warning(
                  this, "Mobshot", tr("The registry could not be changed!"));
            }
...
void ShortcutsWidget::initMsScreenclipCheckbox()
{
    m_registerMsScreenclip =
      new QCheckBox(tr("Register Mobshot as MS-SCREENCLIP application "
                       "(administrator privileges required)"),
                    this);
    m_registerMsScreenclip->setToolTip(
      tr("After registering, you can select Mobshot as the default "
              ...
              if (!registerMsScreenclip()) {
                  QMessageBox::warning(
                    ...
                    tr("The registry could not be changed!") + "\n" +
                      tr("You may start Mobshot as administrator ONCE and "
                         "try again!"));
...
bool ShortcutsWidget::isMsScreenclipRegistered()
{
    QSettings URLAssociations(
      "HKEY_LOCAL_MACHINE\\SOFTWARE\\Mobshot\\Capabilities\\URLAssociations",
      QSettings::NativeFormat);
    QString value = URLAssociations.value("ms-screenclip", "").toString();
    if (value.toLower() != "mobshot")
        return false;

    QSettings RegisteredApplications(
      "HKEY_LOCAL_MACHINE\\SOFTWARE\\RegisteredApplications",
      QSettings::NativeFormat);
    value = RegisteredApplications.value("Mobshot", "").toString();
    if (value.toLower() !=
        QString("SOFTWARE\\Mobshot\\Capabilities").toLower())
        return false;

    QSettings FlameshotShellCmd(
      "HKEY_CURRENT_USER\\Software\\Classes\\Mobshot\\Shell\\Open\\command",
      QSettings::NativeFormat);
...
bool ShortcutsWidget::registerMsScreenclip()
{
    QSettings URLAssociations(
      "HKEY_LOCAL_MACHINE\\SOFTWARE\\Mobshot\\Capabilities\\URLAssociations",
      QSettings::NativeFormat);
    URLAssociations.setValue("ms-screenclip", "Mobshot");
    ...
    QSettings RegisteredApplications(
      "HKEY_LOCAL_MACHINE\\SOFTWARE\\RegisteredApplications",
      QSettings::NativeFormat);
    RegisteredApplications.setValue("Mobshot",
                                    "SOFTWARE\\Mobshot\\Capabilities");
...
bool ShortcutsWidget::unregisterMsScreenclip()
{
    QSettings FlameshotShellCmd("HKEY_CURRENT_USER\\Software\\Classes",
                                QSettings::NativeFormat);
    FlameshotShellCmd.remove("Mobshot");
    FlameshotShellCmd.sync();
    if (QSettings::AccessError == FlameshotShellCmd.status()) {
        return false;
    }
    ...
    QSettings RegisteredApplications(
      "HKEY_LOCAL_MACHINE\\SOFTWARE\\RegisteredApplications",
      QSettings::NativeFormat);
    RegisteredApplications.remove("Mobshot");
    RegisteredApplications.sync();
    if (QSettings::AccessError == RegisteredApplications.status()) {
        return false;
    }
